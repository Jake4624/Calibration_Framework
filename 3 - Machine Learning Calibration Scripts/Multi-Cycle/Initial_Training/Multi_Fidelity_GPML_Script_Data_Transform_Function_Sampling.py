...
#%% Range of inputs
Am_range = np.array([0.01,0.05],dtype="float32")
d_ir_range = np.array([0.0001,0.1],dtype="float32")
mu_range = np.array([0,1],dtype="float32")
...
Am_explore = np.linspace(Am_range[0],Am_range[1],N_Pts,dtype="float32")
d_ir_explore = np.linspace(d_ir_range[0],d_ir_range[1],N_Pts,dtype="float32")
mu_explore = np.linspace(mu_range[0],mu_range[1],N_Pts,dtype="float32")

# Make inputs to explore space

x_explore_unscaled = np.zeros((N_Pts**3,3),dtype="float32")
counter = 0
for a in range(len(Am_explore)):
    for b in range(len(d_ir_explore)):
        for c in range(len(mu_explore)):
...
n_explore=1.96
#
y_explore=np.zeros((N_Pts**3,1),dtype="float32")
v_explore=np.zeros((N_Pts**3,1),dtype="float32")
...
if np.array_equal(min_inputs_mean_func,min_inputs_lower_bound_func) == True:
    ...
    input_props=input_props.reshape((1,num_inputs))
    numsim = 1
else:
...
for i in range(numsim):
    mat_props = np.array([130000,98000,34000,130000,98000,34000,1.1e-05,\
                          6.6e-06,5.7142859,0.1308,308.71429,\
                          253,147.14285,130000,98000,34000,0.002,\
                          input_props[i,0],657.14288,1157.1428,900,\
                          0.125,1.4,336.57144,150,2,4.2857141,\
                          input_props[i,1],0.057185717,input_props[i,2]])
...
** \n""")
    ...
    for j in range(numElsets):
        ...
        f.write(str(mat_props[0])+', ')            #C11A
        f.write(str(mat_props[1])+', ')            #C12A
        f.write(str(mat_props[2])+', ')            #C44A
        f.write(str(mat_props[3])+', ')            #C11M
        f.write(str(mat_props[4])+', ')            #C12M
        f.write(str(mat_props[5])+', ')            #C44M
        f.write(str(mat_props[6])+', ')            #alpha_A
        f.write(str(mat_props[7])+'\n')            #alpha_M
        f.write(str(mat_props[8])+', ')            #f_c
        f.write(str(mat_props[9])+', ')            #gamma_0
        f.write(str(mat_props[10])+', ')           #theta_t
        f.write(str(mat_props[11])+', ')           #theta_ref_low
        f.write(str(mat_props[12])+', ')           #lambda_t
        f.write(str(mat_props[13])+', ')           #C11A
        f.write(str(mat_props[14])+', ')           #C12A
        f.write(str(mat_props[15])+'\n')           #C44A
        f.write(str(mat_props[16])+', ')           #gamma_dot_0
        f.write(str(mat_props[17])+', ')           #A_m
        f.write(str(mat_props[18])+', ')           #S0
        f.write(str(mat_props[19])+', ')           #H0
        f.write(str(mat_props[20])+', ')           #SS
        f.write(str(mat_props[21])+', ')           #A_hard
        f.write(str(mat_props[22])+', ')           #QL
        f.write(str(mat_props[23])+'\n ')          #theta_ref_high
        # Putting in euler angles (texture info)
        f.write(str(orient[j,0]) + ',' + \
                         str(orient[j,1]) + ',' + \
                         str(orient[j,2]) + ',')     #Euler Angles (1,2,3)
        f.write(str(mat_props[24])+', ')           #B_sat
        f.write(str(mat_props[25])+', ')           #b1
        f.write(str(mat_props[26])+', ')           #N_GEN_F
        f.write(str(mat_props[27])+', ')           #d_ir
        f.write(str(mat_props[28])+'\n')           #Dir
        f.write(str(mat_props[29])+', \n')         #mu
    f.write("""** ----------------------------------------------
** 
** STEP: Step-1
** 
